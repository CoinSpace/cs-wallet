...
function parseTx(tx, wallet, addresses) {
  ...
  tx = {
    ...
    ins: tx.vin.map((input) => {
      ...
      const type = bitcoin.address.getAddressType(input.addr, wallet.network);
      txInfo.ins[type] = (txInfo.ins[type] || 0) + 1;
      return {
        address: toAddress(wallet.crypto.platform, input.addr),
    ...
    outs: tx.vout.map((output, index) => {
      const addr = output.scriptPubKey.addresses ? output.scriptPubKey.addresses[0] : null;
      if (csFeeAddresses.indexOf(addr) !== -1 && index !== 0) {
        csFee += output.valueSat;
      }
      if (addresses.indexOf(addr) !== -1) {
        outputValue += output.valueSat;
      }
      const type = bitcoin.address.getAddressType(addr, wallet.network);
      txInfo.outs[type] = (txInfo.outs[type] || 0) + 1;
      return {
        address: toAddress(wallet.crypto.platform, addr),
  ...
  tx.size = bitcoin.TransactionBuilder.getVBytes(txInfo);
...
function toAddress(platform, address) {
  if (platform !== 'bitcoin-cash') return address;
  try {
