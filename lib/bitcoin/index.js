...
bitcoin.TransactionBuilder.prototype.addInputUniversal = function(wallet, unspent) {
  ...
  if (unspent.type === 'p2wpkh') {
    var scriptPubKey = bitcoin.address.toOutputScript(unspent.address, network);
    return this.addInput(unspent.txId, unspent.vout, null, scriptPubKey);
  }
};

bitcoin.TransactionBuilder.prototype.signUniversal = function(wallet, vin, unspent) {
  var keyPair = wallet.getPrivateKeyForAddress(unspent.address);
