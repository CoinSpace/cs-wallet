...
function createTx(wallet, to, value, fee) {
  ...
  return {
    sign() {
      if (wallet.crypto.platform === 'bitcoin-cash' || wallet.crypto.platform === 'bitcoin-sv') {
...
async function processTx(wallet, tx) {
  tx.outs.forEach((out) => {
    ...
    const addressType = bitcoin.address.getAddressType(address, wallet.network);
    const account = wallet.accounts[addressType];
    if (!account) return;
    const nextAddress = wallet.getNextAddress({ addressType, oldFormat: true });
    const nextChangeAddress = wallet.getNextChangeAddress({ addressType });
    if (nextChangeAddress === address) {
      ...
      wallet.cache.set(`deriveIndex.${addressType}`, account.addresses.length);
    }
  });
...
function refundTx(wallet, historyTx) {
  ...
  historyTx.outs.forEach((output) => {
    ...
    if (account.changeAddresses[account.changeAddresses.length - 1] === output.addr) {
      ...
      wallet.cache.set(`deriveIndex.${output.type}`, account.addresses.length);
    }
  });
  if (isOutcoming) {
    wallet.balance += historyTx.fee;
  }
  const index = wallet._txIds.indexOf(historyTx.id);
  if (index !== -1) wallet._txIds.splice(index, 1);
}
...
function createImportTx(wallet, options) {
  ...
  feeUtils.addCsFee(wallet, builder, options.amount);
...
function getImportTxOptions(wallet, privateKey) {
  const addresses = [];
  wallet.network.addressTypes.forEach((type) => {
    // https://github.com/bitcoin/bips/blob/master/bip-0143.mediawiki#Restrictions_on_public_key_type
    if (!privateKey.compressed && ['p2sh', 'p2wpkh'].includes(type)) return;
    addresses.push(wallet.getAddress(privateKey.getPublicKeyBuffer(), type));
  });
...
function createReplacement(wallet, tx) {
  ...
  if (!hasChangeAddress) {
    fee += feePerByte * bitcoin.TransactionBuilder.OUT_SIZES[wallet.addressType];
  }

  utxos.some((unspent) => {
    builder.addInputUniversal(wallet, unspent);
    if (builder.inputs.length > tx.ins.length) {
      fee += feePerByte * bitcoin.TransactionBuilder.IN_SIZES[unspent.type];
    }
    accum += unspent.value;
    subTotal = value + fee;
