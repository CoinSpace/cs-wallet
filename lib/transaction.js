...
function processTx(wallet, tx) {
  if (tx.replaceByFeeTx && tx.replaceByFeeTx.changeAddress) {
    var account = wallet.accounts[tx.replaceByFeeTx.changeAddressType];
    account.changeAddresses.push(tx.replaceByFeeTx.changeAddress);
  }
...
function refundTx(wallet, historyTx) {
  ...
  historyTx.outs.forEach(function(output) {
    ...
    var changeAddressIndex = account.changeAddresses.indexOf(output.addr);
    var addressIndex = account.addresses.indexOf(output.addr);
    if (changeAddressIndex !== -1) {
      account.changeAddresses.splice(changeAddressIndex, 1);
  ...
  utxos.some(function(unspent) {
    ...
    if (accum >= subTotal) {
      ...
      if (change > wallet.network.dustThreshold) {
        ...
        tx.changeAddressType = wallet.addressType;
      }
      return true;
    }
  });
