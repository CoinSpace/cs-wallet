...
function createTx(wallet, to, value, fee) {
  ...
  var utxos = wallet.unspents.filter(function(unspent) {
  ...
  validate.postCreateTx(value, accum, wallet.getBalance(), fee);

  if (wallet.networkName === 'bitcoincash') {
    ...
    unspents.forEach(function(unspent, i) {
      builder.sign(i, wallet.getPrivateKeyForAddress(unspent.address), null, hashType, unspent.value);
    });
  } else {
    unspents.forEach(function(unspent, i) {
      builder.sign(i, wallet.getPrivateKeyForAddress(unspent.address));
    });
  }
...
function sendTx(wallet, tx, done) {
  wallet.api.transactions.propagate(tx.toHex()).then(function() {
    var delay = wallet.networkName === 'dogecoin' ? 1500 : 1; // wait 1.5s for dogecoin
    setTimeout(function() {
...
function processTx(wallet, tx, done) {
  var foundUsed = true;
  while (foundUsed) {
    foundUsed = addToAddresses(wallet.getNextAddress(), wallet.getNextChangeAddress());
  }
...
function getImportTxOptions(wallet, privateKey) {
  ...
  return wallet.api.addresses.unspents([address]).then(function(unspents) {
    unspents = unspents.filter(function(unspent) {
      return unspent.confirmations >= wallet.minConf;
    });
