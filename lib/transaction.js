...
function createTx(wallet, to, value, fee) {
  ...
  var utxos = wallet.getUnspentsForTx();
  var accum = 0;
  var subTotal = value;
  ...
  var usedAllUnspents = builder.inputs.length === wallet.unspents.length;
  validate.postCreateTx(value, accum, wallet.getBalance(), usedAllUnspents, fee);
