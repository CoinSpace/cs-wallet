...
function getCsFee(wallet, value) {
  ...
  if (wallet.csMinFee) {
    var exportTxFee = 192 * network.feePerByte;
...
function estimateFees(wallet, value, unspents) {
  ...
  if (unspents != null) {
    ...
    utxos = wallet.getUnspentsForTx({unspents: unspents});
  } else {
...
function getMaxAmounts(wallet) {
  var balance = wallet.getUnspentsForTx().reduce(function(sum, unspent) {
