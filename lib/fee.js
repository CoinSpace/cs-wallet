...
function estimateFees(wallet, value, { unspents, privateKey } = {}) {
  if (typeof value !== 'number') value = parseInt(value);
  ...
  for (let i = 0; i < wallet.feeRates.length; i++) {
    ...
    if (!unspents && maxAmount && value >= maxAmount.value) {
      ...
      const estimatedFee = estimate(wallet, feeRate.value, value, { unspents: utxos, privateKey });
      if (estimatedFee) {
        fees.push({
          name: feeRate.name,
...
function estimate(wallet, feeRate, value, { unspents, privateKey }) {
  const { network } = wallet;
  const txInfo = {
    ins: {},
    outs: {},
    outsLessDustSoftThreshold: 0,
    isPrivateKeyUncompressed: privateKey && !privateKey.compressed,
  };
