...
function getAPI(network, apiWrapperUrl) {
  var API = null;
  var proxy = null;
  if ((network === 'bitcoin' || network === 'testnet') && process.env.API_WRAPPER === 'insight')  {
    // insight is only for bitcoins
    API = require('cs-insight')
    proxy = process.env.INSIGHT_PROXY_URL
  } else {
    API = require('cs-blockr')
    proxy = process.env.BLOCKR_PROXY_URL
  }
  return new API(network, proxy, apiWrapperUrl)
}
...
function Wallet(externalAccount, internalAccount, networkName, done, unspentsDone, balanceDone) {
  ...
  this.api = getAPI(networkName, process.env.API_WRAPPER_URL)
...
Wallet.deserialize = function(json) {
  ...
  wallet.api = getAPI(deserialized.networkName, process.env.API_WRAPPER_URL)
