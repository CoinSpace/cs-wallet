...
class Wallet {
  constructor(options) {
    ...
    for (const type of this.network.addressTypes) {
      if (type === 'p2pkh') {
        this.settings.bip44 = this.settings.bip44 || (options.fixbip44 ? this.network.bip44 : "m/0'");
      } else if (type === 'p2sh') {
    ...
    if (options.seed) {
      ...
      for (const type of this.network.addressTypes) {
        let base;
        if (type === 'p2pkh') {
          base = hdkey.derive(this.settings.bip44);
        } else if (type === 'p2sh') {
          base = hdkey.derive(this.settings.bip49);
        } else if (type === 'p2wpkh') {
          base = hdkey.derive(this.settings.bip84);
        }
        this.accounts[type] = {
      ...
      for (const type of this.network.addressTypes) {
        ...
        const base = HDKey.fromExtendedKey(extendedKey, this.network.bip32);
        this.accounts[type] = {
          external: base.deriveChild(0),
          internal: base.deriveChild(1),
          base,
