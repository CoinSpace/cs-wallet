...
function Wallet(options) {
  ...
  try {
    if (typeof hdkey === 'string') {
      this.hdkey = HDKey.fromExtendedKey(hdkey);
    } else {
      this.hdkey = hdkey;
    }
    assert(this.hdkey != null, 'hdkey cannot be null');
  } catch (err) {
    return done(err);
  }
  ...
  this.accounts = network.addressTypes.reduce(function(result, type) {
    var accountZero;
    if (type === 'p2pkh') {
      accountZero = hdkey.derive("m/0'");
    } else if (type === 'p2sh') {
      accountZero = hdkey.derive(network.bip49);
    } else if (type === 'p2wpkh') {
      accountZero = hdkey.derive(network.bip84);
    }
    result[type] = {
      external: accountZero.deriveChild(0),
      internal: accountZero.deriveChild(1),
      addresses: [],
      changeAddresses: []
    };
    return result;
...
Wallet.deserialize = function(json) {
  ...
  wallet.hdkey = HDKey.fromExtendedKey(deserialized.hdkey, network.bip32);
  var accountZero = wallet.hdkey.derive("m/0'");
  wallet.accounts = {
    p2pkh: {
      external: accountZero.deriveChild(0),
      internal: accountZero.deriveChild(1),
      addresses: deriveAddresses(accountZero.deriveChild(0), network, deserialized.addressIndex),
      changeAddresses: deriveAddresses(accountZero.deriveChild(1), network, deserialized.changeAddressIndex)
    }
  };
