...
function Wallet(options) {
  ...
  Promise.all([
    ...
    if (results[0]) {
      that.csFee = results[0].fee;
      that.csMinFee = results[0].minFee;
      that.csMaxFee = results[0].maxFee;
      that.csFeeAddresses = results[0].addresses;
    }
    var data = results[1];
...
Wallet.prototype.addCsFee = function(builder, value) {
  if (this.csFeeOff) return 0;
  if (!this.csFee || !this.csMinFee || !this.csMaxFee) return 0;
  if (this.csFeeAddresses.length === 0) return 0;

  var network = bitcoin.networks[this.networkName];
  var fee = Math.max(Math.floor(value * this.csFee), this.csMinFee);
  fee = Math.min(this.csMaxFee, fee);
  ...
  fee += exportTxFee;
  builder.addOutput(this.csFeeAddresses[0], fee);
  return fee;
};
...
Wallet.prototype.estimateFees = function(to, value, feeRates, unspents) {
  ...
  for (var i = 0; i < feeRates.length; i++) {
    var builder = new bitcoin.TransactionBuilder(network);
    builder.addOutput(to, value);
    var csFee = that.addCsFee(builder, value);
    ...
    utxos.some(function(unspent) {
      ...
      estimatedFee = estimateFeePadChangeOutput(builder.buildIncomplete(), network, feeRates[i]) + csFee;
...
Wallet.prototype.sendTx = function(tx, done) {
  ...
  this.api.transactions.propagate(tx.toHex()).then(function() {
    var delay = that.networkName === 'dogecoin' ? 1500 : 1; // wait 1.5s for dogecoin
    setTimeout(function() {
...
Wallet.prototype.processTx = function(tx, done) {
  ...
  fetchTransactions(that.api, allAddresses, [tx.getId()], that.csFeeAddresses).then(function(historyTxs) {
    var historyTx = historyTxs[0];
    if (!historyTx) return done(null, null);
    that.balance += historyTx.amount;
    var minusFees = false;
