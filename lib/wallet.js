...
function Wallet(options) {
  ...
  Promise.all([
    ...
    network.addressTypes.reduce(function(promise, type) {
      return promise.then(function(unspentAddresses) {
        return discoverAddresses(
          ...
          function(node) {
            return getAddress(node, networkName, type);
          }
        ).then(function(data) {
    ...
    if (results[1]) {
      ...
      var firstAddress = getAddress(that.externalAccount.deriveChild(0), that.networkName, 'p2pkh');
...
Wallet.prototype.getNextChangeAddress = function() {
  ...
  return getAddress(node, this.networkName, this.addressType);
};
...
Wallet.prototype.getNextAddress = function(oldFormat) {
  ...
  var address = getAddress(node, this.networkName, this.addressType);
...
Wallet.prototype.getPrivateKeyForAddress = function(address) {
  ...
  var index;
  var network = bitcoin.networks[this.networkName];
  for (var i = 0; i < network.addressTypes.length; i++) {
    var type = network.addressTypes[i];
...
function getAddress(node, networkName, type) {
  ...
  if (type === 'p2sh') {
    ...
    return bitcoin.address.fromOutputScript(scriptPubKey, bitcoin.networks[networkName]);
  }
  if (type === 'p2wpkh') {
    var bech32 = bitcoin.networks[networkName].bech32;
