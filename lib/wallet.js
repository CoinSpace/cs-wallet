...
function getAPI(network, proxy, apiWrapperUrl) {
  var API = null;
  if (network === 'bitcoin' || network === 'testnet') {
    // insight is only for bitcoins
    API = process.env.API_WRAPPER === 'insight' ? require('cs-insight') : require('cs-blockr')
  } else {
...
function Wallet(externalAccount, internalAccount, networkName, done, unspentsDone, balanceDone) {
  ...
  this.api = getAPI(networkName, process.env.PROXY_URL, process.env.API_WRAPPER_URL)
...
Wallet.deserialize = function(json) {
  ...
  wallet.api = getAPI(deserialized.networkName, process.env.PROXY_URL, process.env.API_WRAPPER_URL)
