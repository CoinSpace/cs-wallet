...
Wallet.prototype.load = function(options) {
  ...
  Promise.all([
    ...
    return fetchUnspents(that, unspentAddresses).then(function(utxos) {
      that.unspents = utxos;
...
Wallet.prototype.loadTxs = function() {
  ...
  return promise.then(function() {
    ...
    return fetchTransactions(that, allAddresses, txIds).then(function(txs) {
...
Wallet.prototype.unlock = function(seed) {
  var that = this;
  var hdkey = HDKey.fromMasterSeed(Buffer.from(seed, 'hex'), that.network.bip32);
  Object.keys(that.accounts).forEach(function(type) {
    var base;
    if (type === 'p2pkh') {
      base = hdkey.derive("m/0'");
