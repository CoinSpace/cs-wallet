...
function getAPI(apiNode, network) {
...
class Wallet {
  constructor(options) {
    ...
    const { seed, crypto, cache } = options;
    let { publicKey } = options;

    const { platform } = crypto;
    const network = bitcoin.networks[platform];
    ...
    this.api = getAPI(this.apiNode, network);
    ...
    if (this.cache.has('addressType')) {
      this._addressType = this.cache.get('addressType');
    } else {
      this._addressType = ['bitcoin', 'litecoin'].includes(platform) ? 'p2wpkh' : 'p2pkh';
    }

    this.addressTypes = network.addressTypes;
    ...
    this.replaceByFeeEnabled = platform === 'bitcoin';
    this.maxAmounts = [];
    this.csFee = 0;
    this.csMinFee = 0;
    this.csMaxFee = 0;
  ...
  getNextAddress(oldFormat) {
    const cacheKey = `deriveIndex.${this.addressType}`;
    let deriveIndex;
    const account = this.accounts[this.addressType];
    if (this.cache.has(cacheKey)) {
      deriveIndex = this.cache.get(cacheKey);
    } else {
      deriveIndex = account.addresses.length;
    }
  ...
  static deserialize(json) {
    ...
    wallet.cache = { get: () => {}, has: () => {}, set: () => {}, unset: () => {} };
